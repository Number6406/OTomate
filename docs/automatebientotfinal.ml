...
let toXML(tradautomate A) =
  let writefile = open_out "AutomateenXML.txt" in
...
let rec ecrireListe(A) =
  match liste with 
    |[]-> ;
    |(x,y,z,t)::s->
          fprintf writefile "<case>";
        	fprintf writefile "<ligne>%d</ligne>" x;
        	fprintf writefile "<colonne>%d</colonne>" y;
        	fprintf writefile "<transition>%d</transition>" z;
        	fprintf writefile "<action>%d</action>" t;
        	fprintf writefile "</case>";
	ecrireListe(s);
;;

let rec xml (liste:(int*int*int*int) list) =
  let writefile = 
  open_out "AutomateenXML.xml" in
