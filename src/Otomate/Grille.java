...
//Retourne une liste de 6 entiers repr�sentant les differentes conditions
	...
	public void setP(boolean pie, int x, int y) {
		g[x][y].setPiegee(pie);
	}
	...
	public static void Placements(List<Joueur> J) {
		int l = coinsAutomates.size();
		List<$Personnage> list = new LinkedList<>();
		int i, j, k, nbCond = J.get(0).getPersonnagesI(0).getAutomate().nbconditions(); // nbCond
																						// contient
																						// le
																						// nombre
																						// de
		...
		for (i = 0; i < l; i++) {
			for (j = i + 1; j < l; j++) {
				if (coinsAutomates.get(i).abs < coinsAutomates.get(j).abs
						&& coinsAutomates.get(i).abs + list.get(i).getPosition().getX() < coinsAutomates.get(j).abs) {
				} else if (coinsAutomates.get(i).getX() < coinsAutomates.get(j).getX()
						&& coinsAutomates.get(i).getY() < coinsAutomates.get(j).getY()
						&& coinsAutomates.get(i).getY() + nbCond < coinsAutomates.get(j).getY()) {
				} else if (coinsAutomates.get(i).getX() < coinsAutomates.get(j).getX()
						&& coinsAutomates.get(i).getY() > coinsAutomates.get(j).getY()
						&& coinsAutomates.get(i).getY() > coinsAutomates.get(j).getY() + nbCond) {
				} else if (coinsAutomates.get(i).getX() > coinsAutomates.get(j).getX() && coinsAutomates.get(i)
						.getX() > coinsAutomates.get(j).getX() + list.get(j).getPosition().getX()) {
				} else if (coinsAutomates.get(i).getX() > coinsAutomates.get(j).getX()
						&& coinsAutomates.get(i).getY() < coinsAutomates.get(j).getY()
						&& coinsAutomates.get(i).getY() + nbCond < coinsAutomates.get(j).getY()) {
				} else if (coinsAutomates.get(i).getX() > coinsAutomates.get(j).getX()
					...
					System.out.println(
							"Erreur les automates se superposent on a un probleme pour la generation de leur coordonnees : erreur dans goAutomates !");
			}
		}
		int jdeb, kdeb;
		int x = 0, s, y;
		...
		while (x < J.size()) {
			...
			while (y < s && i < l) {
				// for(i=0;i<l;i++){ //nombre d'automates a placer
				jdeb = coinsAutomates.get(i).getX();
				for (j = jdeb; j < J.get(x).getPersonnagesI(y)
						.nbEtat()/* nbetats.get(i)+jdeb */; j++) { // parcours
																	// de lignes
					kdeb = coinsAutomates.get(i).getY();
					for (k = kdeb; k < nbCond + kdeb; k++) { // parcours des
																// colonnes
						g[j][k] = J.get(x).getPersonnagesI(y).a.getActions(k - kdeb, j - jdeb);
					}
				}
	...
	public static List<Coordonnees> goAutomates(List<$Personnage> list, int dimh, int dimv) {
		...
		for (k = 0; k < list.size(); k++) {
			...
			for (i = 0; i < k; i++) {
				if (newc.getX() == res.get(i).getX() && newc.getY() == res.get(i).getY()) {
					i = k;
					k--; // la c'est pour refaire le meme tour puisque la case
							// est deja occupee
				}
			}
			if (j == k) { // c'est pour verifier qu'on est pas tomba dans le if
							// et que c'est bon la case est dispo
				res.add(newc);
			}
		}
		return res;
	...
	// On rappelle que l'"origine" du repere de la grille est en haut gauche
	// donc un deplacement au nord = -1 en ord et +1 pour aller vers le sud
	// cependant
