...
public class TestPiege {
	...
	public static void initialisergrille(List<Joueur> l, Grille G) {
        ...
        for(i=0; i<G.tailleX(); i++){
            for(j=0; j<G.tailleY(); j++){
                k = Grille.random(0, 1);        //car 15 actions possibles numerotees de 0 a 14 
                G.set(k, i, j);
            }
        }
    ...
    public static void debutPartie(int numeroUnivers, int nZombie, int nbPersoParZombie, List<List<String>> xmls, List<Color> couleurs) {
    	...
    	Jeu.joueurs.get(0).getPersonnagesI(0).setPosition(new Coordonnees(3,6));
    	Jeu.joueurs.get(1).getPersonnagesI(0).setPosition(new Coordonnees(6,6));
