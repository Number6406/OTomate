	...
	public static void main(String[] pArgs) throws InterruptedException {
		...
		while (!finPartie()) {
			...
			for (int i = 0; i < refPersos.size(); i++) {
				...
				if (joueurs.get(j).getPersonnagesI(p) instanceof Gentil) {
					...
					while (gentilperso.getParalysie() > 0) {
						if (gentilperso.getEfdrogue() != 0) {
							...
							// ((Gentil) p).efdrogue --; PENSER A LE METTRE A LA
							// FIN DE LA GRANDE BOUCE DE TOUR
						} else {
						...
						((Gentil) joueurs.get(j).getPersonnagesI(p))
								.setParalysie(((Gentil) joueurs.get(j).getPersonnagesI(p)).getParalysie() - 1);
						Thread.sleep(200);
					}
				} else {
					tempHistorique = joueurs.get(j).getPersonnagesI(p).jouer(listCond, plateau, listCont, joueurs);
				...
				// tempHistorique sera la chaîne renvoyée par l'action d'un
				// joueu
				// $Personnage persoCourant =
				// joueurs.get(refPersos.get(i)/100).getPersonnagesI(refPersos.get(i)-(refPersos.get(i)/100));
				// tempHistorique = persoCourant.jouer();
				// tempHistorique sera la chaine renvoyee par l'action d'un
				// joueur
			}
			Thread.sleep(200);
